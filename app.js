...
async function loadQueueInfo(date) {
    try {
        ...
        document.getElementById('yourPosition').textContent = `${queueSize + 1}º`;
...
document.getElementById('joinQueueBtn').addEventListener('click', async () => {
    ...
    try {
        ...
        if (!existingQueue.empty) {
            alert('Você já está na fila para esta data!');
            return;
        }
...
function showQueueModal(peopleAhead) {
    ...
    // Escutar mudanças na fila em tempo real
        ...
        const unsubscribe = db.collection('queues')
            ...
            .onSnapshot((snapshot) => {
                if (!snapshot.empty) {
                    ...
                    // Contar quantas pessoas estão na frente
                    db.collection('queues')
                        ...
                        .then(queueSnapshot => {
                            ...
                            if (ahead === 0) {
                                counter.textContent = '🎉';
                                counter.nextElementSibling.textContent = 'É a sua vez!';
                            }
                        });
                }
            });
...
document.getElementById('addBarberForm').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        await db.collection('barbers').add({
            name: name,
            availability: [1, 2, 3, 4, 5], // Segunda a sexta por padrão
            createdAt: firebase.firestore.FieldValue.serverTimestamp()
        });
